...
Memory is provided via the `memory` MCP server (see `.vscode/mcp.json`) so agents can retain context across sessions.
...
### Reusable “Skills” (recommended)

If you find yourself copying the same command blocks and checklists across multiple agents (compose bring-up, health checks, release preflight, etc.), factor them into **skills**.

In this repo, skills live under:

- `.github/agents/reference/skills/`

Skills are just small, reusable Markdown snippets that define:

- Inputs (variables to set)
- Commands
- Acceptance criteria

Agents can then reference these skills instead of duplicating long procedures, making them easier to reuse across projects.
...
**Role**: Retrieves/stores long-term context using the `memory` MCP server so tasks stay coherent across sessions.
