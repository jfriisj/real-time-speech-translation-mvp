...
├── implementation/     # Implementation notes (Implementer)
├── retrospectives/     # Lessons learned
├── releases/           # Release/deployment documentation
├── process-improvement/# Workflow evolution artifacts
└── escalations/        # Explicit decision records / blockers
```
...
- **Memory anchors**: MCP memory stores references to documents
- **Version control**: Track evolution of decisions

---

## Reusable Skills (Procedure Library)

As agent suites grow, long procedural blocks tend to be duplicated across multiple `.agent.md` files (for example: Docker Compose bring-up, standard health checks, CPU/GPU assertions, and common smoke tests).

To reduce duplication and make agents easier to reuse across repositories, this repo supports a lightweight “skills” layer:

- Skills are small Markdown files under `.github/agents/reference/skills/`
- A skill defines **Inputs → Commands → Acceptance criteria**
- Agents can reference skills by path instead of embedding large, repo-specific blocks

This keeps agents focused on **role-specific judgment** (what to validate, how to interpret failures) while skills hold the repeatable procedures.
...
```text
NNN-feature-slug-type.md
```

- **NNN**: Sequential number (001, 002, ...)
- **feature-slug**: Descriptive, stable slug used across artifacts (auth-system, api-refactor)
- **type**: Document type. This may be multi-part (e.g. `plan-critique`, `code-audit`).
...
**Rule of thumb**: The *plan* defines the canonical `NNN-feature-slug`, and all downstream artifacts reuse it (QA/UAT/Release/etc.).
...
If you want a quick start, see `agent-output/templates/` for starter documents.
...
This workflow uses the `memory` MCP server (typically `@modelcontextprotocol/server-memory`) to provide **workspace-scoped long-term memory** for GitHub Copilot. Unlike chat history (which is lost between sessions), MCP memory persists across sessions as a local knowledge graph.
...
- **Agent Tools**: Exposes `#memory.search_nodes`, `#memory.open_nodes`, `#memory.create_entities`, and `#memory.add_observations` for agents

### Why MCP memory is useful
...
1. Ensure your workspace config includes a `memory` MCP server (see `.vscode/mcp.json`).
2. Start MCP servers in VS Code so Copilot Chat can access `memory/*` tools.
...
4. **Acknowledge storage**: Always say "Saved progress to MCP memory."
...
#memory.create_entities {
  "entities": [
    {
      "name": "decision:auth-plan-review-001",
      "entityType": "decision",
      "observations": [
        ...
        "Decision: Rate limiting required on all auth endpoints",
        "Decision: Threat model needed for password reset flow",
        "Rationale: Without rate limiting, credential stuffing attacks are trivial",
        "Rationale: Password reset is a common attack vector requiring explicit analysis",
        "Status: Active",
...
**Requires**: The `memory` MCP server configured and running.
...
- Is the `memory` MCP server configured in `.vscode/mcp.json`?
- Are MCP servers started in VS Code?
- Check Copilot/MCP logs for server start errors
...
- Use MCP memory + artifacts to maintain context
...
**Q: Can I use this without MCP memory?**
