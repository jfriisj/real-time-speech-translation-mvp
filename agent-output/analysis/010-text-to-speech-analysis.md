...
| 2026-01-26 | Planner | Re-evaluate after Plan Rev 19 | Added new gaps: intelligibility evidence, roadmap CPU/GPU verification path, speed-control instrumentation, and MinIO retention proof. |
...
- The plan now explicitly expects the Ingress Gateway to own `speaker_id`/`speaker_reference_bytes`, reducing integration ambiguity.
...
- The current reference environment cannot prove the roadmap-level claim of reliable CPU+GPU runtime because GPU inference is deferred to v0.6.0; without a follow-up artifact/epic, there is no evidence pipeline that a GPU profile was tested.
- The plan still defers intelligibility verification (“Subjective intelligibility deferred to staging”), leaving the central value statement unproven for v0.5.0.
...
- Capture evidence that the generated audio is intelligible in the reference environment (e.g., human playback notes, simple intelligibility scoring, or automated transcription) so that the plan’s “intelligible spoken audio” statement is verifiable before v0.5.0 release.
- Document the GPU validation follow-up (plan or epic) that will actually test `onnxruntime-gpu`; until then, treat the roadmap’s “CPU+GPU reliability” claim as pending follow-up and log the responsible artifact.
...
- Verify the MinIO `tts-audio` bucket actually enforces a 24 h lifecycle rule (e.g., `expire-tts-audio`). Capture the rule definition and evidence (CLI/MinIO UI snapshot) to satisfy the plan’s retention assumption.
...
- **Intelligibility proof** (The updated value statement now promises “intelligible spoken audio”, yet the plan still defers human listening to staging. Without inline evidence—automated transcription or recorded playback notes—the value outcome remains an assumption.)
...
5) How can we capture intelligibility evidence for the updated value statement within the reference environment (automated transcription, listening notes, or scoring) so that the plan can close the measurability gap?
6) Where is the follow-up epic/plan that validates the roadmap’s CPU+GPU reliability promise, and what artifacts (GPU image tests, RTF/latency measurements) will be produced under that follow-up?
## Handoff
